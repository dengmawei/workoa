...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$.validator.addMethod("superId_required",function(value){
			var isParent = $("input[name='isParent']:checked").val();
			if(isParent==0){
				if(value!=null && value!=""){
					return true;
...
<body>
	<div id="breadcrumb">
		...
		<a href="javascript:void(0)" class="current">添加模块</a>
	</div>
			...
			<div class="span12">
				<div class="widget-box">
					...
					<div class="widget-content nopadding">
						<form id="moduleForm" action="${ctx }/module/addModule.html?sid=${sid}" method="post" class="form-horizontal">
							<div class="control-group">
								<label class="control-label">模块名</label>
								<div class="controls w50p">
									<input type="text" name="moduleName" maxlength="30" value="${moduleView.moduleName }">
								</div>
							</div>
							<div class="control-group">
								<label class="control-label">模块值</label>
								<div class="controls w50p">
									<input type="text" name="moduleValue" maxlength="30" value="${moduleView.moduleValue }">
								</div>
							</div>
							<div class="control-group">
								<label class="control-label">访问路径</label>
								<div class="controls w50p">
									<input type="text" name="linkUrl" maxlength="50" value="${moduleView.linkUrl }" <c:if test="${moduleView.isParent!=0 }">disabled="disabled"</c:if>>
								</div>
							</div>
							<div class="control-group">
								...
								<div class="controls">
									<c:choose>
										<c:when test="${moduleView.isParent==0 }">
							...
							<c:if test="${moduleView.isParent==0 }">
								<div class="control-group" id="parentList">
									<label class="control-label">父节点</label>
									<div class="controls">
										<input type="hidden" id="superId" name="superId" value="0">
										<select name="superModuleList" id="superModuleList" class="w100">
											<c:if test="${superModuleList!=null}">
												<c:forEach items="${superModuleList }" var="module">
													<c:if test="${moduleView.superId==module.id }">
														<option value="${module.id }" selected="selected">${module.moduleName }</option>
													</c:if>
													<c:if test="${moduleView.superId!=module.id }">
							...
							<div class="control-group">
								...
								<div class="controls w50p">
									<textarea rows="5" cols="20" name="moduleDesc">${moduleView.moduleDesc }</textarea>
								</div>
							</div>
	...
	<div id="subDialog" title="系统提示" class="modal hide fade">
		...
		<div class="modal-footer">
    		...
    		<input type="button" class="btn btn-primary" onclick="addModule()" value="确定">
		</div>
	</div>
</body>
</html>